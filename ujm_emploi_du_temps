 <?php
    ...
    define("NUMBER_WEEK_IN_YEAR", 52); // On sait jamais si ca change un jour :o 
    /* Oui je sais je mélange l'anglais et le francais 
     * un peu partout dans le code je ferais les corrections quand j'ai le gout 
     */
?>
<!DOCTYPE>
<html>
...
if(!preg_match('#<p>(.+)<\/p>#', $contenu, $titre))
  echo "Failure with 'recuperation clef'<br />";
?>
<header id="info">Petite info : Mise a jours du script. Il ne devrait plus jamais y avoir de bug [ Normalement ] ni de maintenance.<br />
Mise en place du code source en temps réel également</header>
<br />

<center>
Promo :
<!-- Oui j'ai regardé les liens un par un pour avoir les codes -->
...
function goto(val) {
    ...
    document.getElementById('image').src = "http://planning.univ-st-etienne.fr/ade/imageEt?identifier=<?php  echo $titre[1];?>&projectId=7&idPianoWeek="+s+"&idPianoDay="+tab+"&idTree="+tree+"&width=1300&height=500&lunchName=REPAS&displayMode=1057855&showLoad=false&ttl=1378793160704&displayConfId=56";
}
</script>
...
<img alt="Emploi du temps" id="image" src="http://planning.univ-st-etienne.fr/ade/imageEt?identifier=<?php  echo $titre[1];?>&projectId=7&idPianoWeek=<?php echo $sv_num_semaine; ?>&idPianoDay=<?php if ($day == -1) echo 0; else echo $day;?>&idTree=3083&width=1300&height=500&lunchName=REPAS&displayMode=1057855&showLoad=false&ttl=1378793160704&displayConfId=56">
<a href="sourcecode.php">Acces au code source en direct</a>
</center>
